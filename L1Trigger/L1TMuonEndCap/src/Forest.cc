...
///        std::sort(e[i].begin(), e[i].end(), compareEvents);
        std::sort(e[i].begin(), e[i].end(), [] (Event* e1, Event* e2) { return e1->data[Event::sortingIndex] < e2->data[Event::sortingIndex]; } );
    }
}
...
void L1TForest::loadFromCondPayload(const L1TMuonEndCapForest::DForest& forest)
{
