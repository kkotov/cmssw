...
void TriggerSystem::configureSystemFromFiles(const char *hwCfgFile, const char *topCfgFile, const char *key) throw() {
...
void TriggerSystem::addProcessor (const char *processor, const char *role, const char *crate, const char *slot) throw (std::runtime_error) {
    // every processor must have a single defined role
    auto p2r = procToRole.find(processor);
    if( p2r != procToRole.end() && p2r->second != role )
        throw std::runtime_error("Processor: '" + string(processor) + "' already exists but with different role: '" + p2r->second + "'");
    else {
...
void TriggerSystem::addDaq(const char *daq, const char *role, const char *crate) throw (std::runtime_error) {
...
void TriggerSystem::addParameter(const char *id, const char *procOrRole, const char *type, const char *value, const char *delim) throw (std::runtime_error) {
    ...
    if( processor != procParameters.end() ){
        ...
        if( setting != processor->second.end() ){
            ...
            processor->second.insert(
                 make_pair(string(id), Parameter(id,procOrRole,type,value,delim))
            );
...
void TriggerSystem::addParameterTable(const char *id, const char *procOrRole, const char *columns, const char *types,  const vector<string>& rows, const char *delim) throw (runtime_error) {
    // some tables forget to specify delimeter and we get an empty string here
    //  force the "," default delimeter
    if( strlen(delim) == 0 ) delim = ",";
    ...
    if( processor != procParameters.end() ){
        ...
        if( setting != processor->second.end() )
            ...
            processor->second.insert(
                 make_pair(string(id), Parameter(id,procOrRole,types,columns,rows,delim))
            );
        // let's run a consistency check
        auto p2r = procToRole.find(procOrRole);
        if( p2r == procToRole.end() )
...
const map<string, Parameter>& TriggerSystem::getParameters(const char *p) const throw(runtime_error) {
    if( !isConfigured )
...
void TriggerSystem::addMask(const char *id, const char *procOrRoleOrDaq) throw(runtime_error) {
    ...
    if( processor != procMasks.end() ){
        ...
        if( mask != processor->second.end() ){
            ...
            processor->second.insert(
                 make_pair(string(id), Mask(id, procOrRoleOrDaq))
            );
        // let's run a consistency check
    ...
    if( role != roleForProcs.end() ){
        ...
        for(auto &proc : role->second){
            ...
            if( processor != procMasks.end() ){
                ...
                procMasks.insert(
                   make_pair(proc, map<string,Mask>( {{id,Mask(id, procOrRoleOrDaq)}} ) )
                );
            }
        }			
       return;
    ...
    if( d2c != daqttcToCrate.end() ){
        ...
        if( processors != crateForProcs.end() ){
            ...
            if( logs )
               *logs << "Warning: no processors in daqttc crate for "
                     << procOrRoleOrDaq << " ... do nothing" << endl;
        return;
    }
    ...
    if( r2d != roleForDaqttcs.end() ){
        for(auto &daq : r2d->second){
            ...
            if( processors != crateForProcs.end() ){
                ...
                if( logs )
                   *logs << "Warning: no processors in daqttc crate " << d2c->second
                         << " for " << procOrRoleOrDaq << " ... do nothing" << endl;
            return;
	}
    }
...
const map<string, Mask>& TriggerSystem::getMasks(const char *p) const throw(runtime_error) {
    if( !isConfigured )
...
bool TriggerSystem::isMasked(const char *p, const char *id) const throw(runtime_error) {
    const std::map<std::string, Mask>& m = getMasks(p);

    auto mask = m.find(id);
    if( mask == m.end() ) return false;

    return true;
}

void TriggerSystem::disableProcOrRoleOrDaq(const char *procOrRoleOrDaq) throw(runtime_error) {
...
bool TriggerSystem::isProcEnabled(const char *p) const throw(runtime_error) {
    if( !isConfigured )
        throw std::runtime_error("TriggerSystem is not configured yet. First call the configureSystem method");
