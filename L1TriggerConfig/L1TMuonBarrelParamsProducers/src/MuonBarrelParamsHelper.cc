...
void MuonBarrelParamsHelper::configFromPy(std::map<std::string, int>& allInts, std::map<std::string, bool>& allBools, std::map<std::string, std::vector<std::string> > allMasks, unsigned int fwVersion, const std::string& AssLUTpath)
{
...
///	write_->l1mudttfparams.reset();
///	write_->l1mudttfqualplut.load();
///	write_->l1mudttfetaplut.load();

	for( int wh=-3; wh<4; wh++ ) {
	   int sec = 0;
	   for(char& c : allMasks["mask_phtf_st1"].at(wh+3) ) {
	        int mask = c - '0';
	        write_->l1mudttfmasks.set_inrec_chdis_st1(wh,sec,mask);
	        sec++;
	    }
	   sec = 0;
	   for(char& c : allMasks["mask_phtf_st2"].at(wh+3) ) {
	        int mask = c - '0';
	        write_->l1mudttfmasks.set_inrec_chdis_st2(wh,sec,mask);
	        sec++;
	    }
	   sec = 0;
	   for(char& c : allMasks["mask_phtf_st3"].at(wh+3) ) {
	        int mask = c - '0';
	        write_->l1mudttfmasks.set_inrec_chdis_st3(wh,sec,mask);
	        sec++;
	    }
	   sec = 0;
	   for(char& c : allMasks["mask_phtf_st4"].at(wh+3) ) {
	        int mask = c - '0';
	        write_->l1mudttfmasks.set_inrec_chdis_st4(wh,sec,mask);
	        sec++;
	    }
	   sec = 0;
	   for(char& c : allMasks["mask_ettf_st1"].at(wh+3) ) {
	        int mask = c - '0';
	        write_->l1mudttfmasks.set_etsoc_chdis_st1(wh,sec,mask);
	        sec++;
	    }
	   sec = 0;
	   for(char& c : allMasks["mask_ettf_st2"].at(wh+3) ) {
	        int mask = c - '0';
	        write_->l1mudttfmasks.set_etsoc_chdis_st2(wh,sec,mask);
	        sec++;
	    }
	   sec = 0;
	   for(char& c : allMasks["mask_ettf_st3"].at(wh+3) ) {
	        int mask = c - '0';
	        write_->l1mudttfmasks.set_etsoc_chdis_st3(wh,sec,mask);
	        //Not used in BMTF - mask
	        write_->l1mudttfmasks.set_inrec_chdis_csc(wh,sec,true);
	        sec++;
	    }
	...
	//write_->l1mudttfextlut.load();
}

void MuonBarrelParamsHelper::configFromDB(l1t::TrigSystem& trgSys)
{
	    ...
	    for(int m=0; m<5; m++)
	    {
	        ...
	        if (it->getRowValue<std::string>("register_path").find(masks[m]) != std::string::npos){
	          ...
	          if((mask&1)>0)  {
	             for(int sec=0; sec<12; sec++){
	              if(masks[m]=="mask_ctrl_N2"){
	                                    write_->l1mudttfmasks.set_inrec_chdis_st1(-3,sec,true);
	                                    write_->l1mudttfmasks.set_etsoc_chdis_st1(-3,sec,true);
	              }
	              if(masks[m]=="mask_ctrl_N1"){
	                                    write_->l1mudttfmasks.set_inrec_chdis_st1(-2,sec,true);
	                                    write_->l1mudttfmasks.set_etsoc_chdis_st1(-2,sec,true);
	              }

	              if(masks[m]=="mask_ctrl_0"){
	                                    write_->l1mudttfmasks.set_inrec_chdis_st1(-1,sec,true);
	                                    write_->l1mudttfmasks.set_inrec_chdis_st1(1,sec,true);
	                                    write_->l1mudttfmasks.set_etsoc_chdis_st1(-1,sec,true);
	                                    write_->l1mudttfmasks.set_etsoc_chdis_st1(1,sec,true);
	              }
	          ...
	          if((mask&2)>0)  {
	            for(int sec=0; sec<12; sec++){
	              if(masks[m]=="mask_ctrl_N2"){
	                                    write_->l1mudttfmasks.set_inrec_chdis_st2(-3,sec,true);
	                                    write_->l1mudttfmasks.set_etsoc_chdis_st2(-3,sec,true);
	              }
	              if(masks[m]=="mask_ctrl_N1"){
	                                    write_->l1mudttfmasks.set_inrec_chdis_st2(-2,sec,true);
	                                    write_->l1mudttfmasks.set_etsoc_chdis_st2(-2,sec,true);
	              }

	              if(masks[m]=="mask_ctrl_0"){
	                                    write_->l1mudttfmasks.set_inrec_chdis_st2(-1,sec,true);
	                                    write_->l1mudttfmasks.set_inrec_chdis_st2(1,sec,true);
	                                    write_->l1mudttfmasks.set_etsoc_chdis_st2(-1,sec,true);
	                                    write_->l1mudttfmasks.set_etsoc_chdis_st2(1,sec,true);
	              }
	              if(masks[m]=="mask_ctrl_P1"){
	                                    write_->l1mudttfmasks.set_inrec_chdis_st2(2,sec,true);
	          ...
	          if((mask&4)>0)  {
	            for(int sec=0; sec<12; sec++){
	              if(masks[m]=="mask_ctrl_N2"){
	                                    write_->l1mudttfmasks.set_inrec_chdis_st3(-3,sec,true);
	                                    write_->l1mudttfmasks.set_etsoc_chdis_st3(-3,sec,true);
	              }
	              if(masks[m]=="mask_ctrl_N1"){
	                                    write_->l1mudttfmasks.set_inrec_chdis_st3(-2,sec,true);
	                                    write_->l1mudttfmasks.set_etsoc_chdis_st3(-2,sec,true);
	              }

	              if(masks[m]=="mask_ctrl_0"){
	                                    write_->l1mudttfmasks.set_inrec_chdis_st3(-1,sec,true);
	                                    write_->l1mudttfmasks.set_inrec_chdis_st3(1,sec,true);
	                                    write_->l1mudttfmasks.set_etsoc_chdis_st3(-1,sec,true);
	                                    write_->l1mudttfmasks.set_etsoc_chdis_st3(1,sec,true);
	              }
	              if(masks[m]=="mask_ctrl_P1"){
	                                    write_->l1mudttfmasks.set_inrec_chdis_st3(2,sec,true);
	          ...
	          if((mask&8)>0)  {
	            for(int sec=0; sec<12; sec++){
	              if(masks[m]=="mask_ctrl_N2"){
	                                    write_->l1mudttfmasks.set_inrec_chdis_st4(-3,sec,true);
	              }
	              if(masks[m]=="mask_ctrl_N1"){
	                                    write_->l1mudttfmasks.set_inrec_chdis_st4(-2,sec,true);
	              }

	              if(masks[m]=="mask_ctrl_0"){
	                                    write_->l1mudttfmasks.set_inrec_chdis_st4(-1,sec,true);
	                                    write_->l1mudttfmasks.set_inrec_chdis_st4(1,sec,true);
	              }
	              if(masks[m]=="mask_ctrl_P1"){
	                                    write_->l1mudttfmasks.set_inrec_chdis_st4(2,sec,true);
...
int MuonBarrelParamsHelper::load_pt(std::vector<MuonBarrelParamsHelper::LUT>& pta_lut,
...
int MuonBarrelParamsHelper::load_phi(std::vector<LUT>& phi_lut,
...
/*
int MuonBarrelParamsESProducer::getPtLutThreshold(int pta_ind, std::vector<int>& pta_threshold) const {
...
//
int MuonBarrelParamsHelper::load_ext(std::vector<L1TMuonBarrelParams::LUTParams::extLUT>& ext_lut,
                                            unsigned short int nbit_phi,
                                            unsigned short int nbit_phib) {
